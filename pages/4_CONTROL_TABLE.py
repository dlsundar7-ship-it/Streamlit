...
with st.container(border=True):
    ...
    with filter_col1:
        ...
        selected_table_desc = st.multiselect(
            "Table Description",
            sorted(
                st.session_state.table_load_data["TABLE_DESCRIPTION"]
                .fillna("NA")
                .astype(str)
                .str.strip()
                .replace("", "NA")
    ...
    with filter_col2:
        ...
        selected_table_name = st.multiselect(
            "Table Name",
            sorted(
                st.session_state.table_load_data["TABLE_NAME"]
                .fillna("NA")
                .astype(str)
                .str.strip()
                .replace("", "NA")
    ...
    with filter_col3:
        ...
        selected_table_type = st.multiselect(
            "Table Type",
            sorted(
                st.session_state.table_load_data["TABLE_TYPE"]
                .fillna("NA")
                .astype(str)
                .str.strip()
                .replace("", "NA")
    ...
    with filter_col4:
        ...
        selected_report_module = st.multiselect(
            "Report Module",
            sorted(
                st.session_state.table_load_data["REPORT_MODULE"]
                .fillna("NA")
                .astype(str)
                .str.strip()
                .replace("", "NA")
...
with st.container(border=True):
    ...
    save_col, cancel_col, _ = st.columns([1.6, 1, 9])
